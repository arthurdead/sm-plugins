...
#define MAX_ITEM_CAT_NAME 64

#define MAX_ITEM_NAME 64
#define MAX_ITEM_DESC  64
#define MAX_ITEM_CLASS 64

#define MAX_ITEM_SETT_NAME 64
#define MAX_ITEM_SETT_VALUE 64

enum struct ItemCategoryInfo
{
	int id;
	char name[MAX_ITEM_CAT_NAME];
	ArrayList items;
	Menu shop_menu;
}
...
static void add_item_to_player_inv_menu(int client, int id, int idx)
{
	...
	if(!player_inventory_categories[client].GetArray(catinfo.name, plrinvcat, sizeof(PlayerInventoryCategory))) {
		...
		pack_int_in_str(info.category, str, 0);

		player_inventory_menu[client].AddItem(str, catinfo.name);

		player_inventory_categories[client].SetArray(catinfo.name, plrinvcat, sizeof(PlayerInventoryCategory));
	}
...
#if defined DEBUG
	PrintToServer("%i, %i", client, player_currency[client]);
#endif
...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_remei", sm_remei, ADMFLAG_ROOT);
...
static void cache_data(Database db, any data, int numQueries, DBResultSet[] results, any[] queryData)
{
	...
	shop_menu.AddItem("", "", ITEMDRAW_DISABLED);
	...
	int len = snap.Length;
	for(int i = 0; i < len; ++i) {
		snap.GetKey(i, classname, MAX_ITEM_CLASS);
...
static void cache_categories(DBResultSet set)
{
	...
	info.id = set.FetchInt(0);
	set.FetchString(1, info.name, MAX_ITEM_NAME);
...
static void cache_items(DBResultSet set)
{
	...
	set.FetchString(2, info.name, MAX_ITEM_NAME);
	set.FetchString(3, info.desc, MAX_ITEM_DESC);
	set.FetchString(4, info.classname, MAX_ITEM_CLASS);

	info.price = set.FetchInt(5);
	...
	info.shop_menu.AddItem("", "", ITEMDRAW_SPACER);

	if(info.price != -1) {
		char price_str[15];
		IntToString(info.price, price_str, sizeof(price_str));
...
#if defined DEBUG
	PrintToServer("%s", info.name);
	PrintToServer("  %s", info.classname);
	PrintToServer("  %i, %i, %i", info.id, idx, info.equipable);
#endif
}
...
static void cache_items_settings(DBResultSet set)
{
	...
	char name[MAX_ITEM_SETT_NAME];
	set.FetchString(1, name, MAX_ITEM_SETT_NAME);

	char value[MAX_ITEM_SETT_VALUE];
	set.FetchString(2, value, MAX_ITEM_SETT_VALUE);
...
#if defined DEBUG
	PrintToServer("%i, %i", client, amount);
#endif
...
#if defined DEBUG
	PrintToServer("%i, %i, %i, %i", client, id, item_id, equipped);
#endif
	...
	switch(action) {
		...
		case MenuAction_Cancel: {
			if(param2 == MenuCancel_ExitBack) {
				...
				ItemCategoryInfo catinfo;
				categories.GetArray(catidx, catinfo, sizeof(ItemCategoryInfo));
	...
	switch(action) {
		...
		case MenuAction_DrawItem: {
			switch(param2) {
				...
				case 1: return ITEMDRAW_SPACER;
				case 2: return ITEMDRAW_DISABLED;
				case 3: {
					char str[10];
					menu.GetItem(param2, str, sizeof(str));
